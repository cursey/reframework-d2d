...
void on_ref_lua_state_created(lua_State* l) try {
    ...
    d2d["fill_circle"] = [](float x, float y, float rX, float rY, unsigned int color) {
        g_plugin->cmds->fill_circle(x, y, rX, rY, color);
    };
    d2d["circle"] = [](float x, float y, float rX, float rY, float thickness, unsigned int color) {
